...
namespace CalDav_tests
{
    public class PropfindTests
    {
        ...
        private const string ColUrl = "collections/groups/public/C111";
        ...
        private IPropfindMethods GetPropMethods(CalDavContext db)
        {
            return new CalDavPropfind(new CalendarCollectionRepository(db), new CalendarResourceRespository(db),
                new CalendarHomeRepository(db));
        }

        private async Task<Principal> GetPrincipal(CalDavContext db)
        {
            return await db.Principals.FirstAsync();
        }

        private CalDav GetCalDav(FileManagement fs, CalDavContext db)
        {
            var principalRepo = new PrincipalRepository(db);
            var uhAuth = new UhCalendarAuthentication(principalRepo);
            var aclPropfind = new ACLProfind(uhAuth);
            var resourceRepo = new CalendarResourceRespository(db);
            var collectionRepo = new CalendarCollectionRepository(db);
            var permissionGuard = new PermissionsGuard(resourceRepo, collectionRepo);
            var reportPrecon = new ReportPreconditions(permissionGuard, uhAuth);
            var collectionReport = new CollectionReport(resourceRepo, reportPrecon);
            var homeRepo = new CalendarHomeRepository(db);

            return new CalDav(fs, aclPropfind, collectionReport, collectionRepo, resourceRepo, principalRepo,
                permissionGuard, homeRepo, uhAuth);
        }
...
//             }
            ...
            var propMethods = GetPropMethods(db);
            //             propMethods.PropNameMethod("foo@gmail.com", "Foocollection", null, 0, response);
            await propMethods.PropNameMethod(ColUrl, null, null, response);
        ...
        [Fact]
        public async Task AllVisibleNotEmpty()
        {
            var db = MockDatabase();
            ...
            var propMethods = GetPropMethods(db);
            //await propMethods.AllPropMethod("foo@gmail.com", "Foocollection", null, 0, null, response);
            await propMethods.AllPropMethod(ColUrl, null, null, null, response);
            IXMLTreeStructure prop;
            response.GetChildAtAnyLevel("prop", out prop);
            Assert.NotNull(prop);
            Assert.True(prop.Children.Count > 0);
        ...
        [Fact]
        public async void PropCollectionEmptyBody()
        {
            var db = MockDatabase();
            ...
            var principal = await GetPrincipal(db);
            await propMethods.PropMethod(ColUrl, null, null, null, response, principal);
            IXMLTreeStructure prop;
            response.GetChildAtAnyLevel("prop", out prop);
            Assert.Null(prop);
            //Assert.True(prop.Children.Count == 0);
        ...
        private CalDavContext MockDatabase()
        {
            ...
            var optionsBuilder = new DbContextOptionsBuilder<CalDavContext>().UseInMemoryDatabase();
...
//            optionsBuilder.UseInMemoryDatabase();
            ...
            var calendarCollectionRepo = new CalendarCollectionRepository(db);
            var calendarResourceRepo = new CalendarResourceRespository(db);
            var calendarHomeRepo = new CalendarHomeRepository(db);

            var principal = new Principal()
            {
                PrincipalStringIdentifier = "pepe",
                PrincipalUrl = "/principals/users/pepe/"
            };
            ...
            {
                Name = "HomeCollection",
                Url = "/collections/groups/public/",
                Properties = new List<Property>()
            };

            calendarHomeRepo.Add(calendarHome);


            var calendarResource = new CalendarResource
            {
...
//                    }
            ...
            calendarResourceRepo.Add(calendarResource);
...
//                    }
            ...
            collection.CalendarResources = new List<CalendarResource> {calendarResource};
            calendarCollectionRepo.Add(collection);


            calendarHome.CalendarCollections = new List<CalendarCollection> {collection};

            principal.User = user;
            principal.CalendarHome = calendarHome;
        ...
        [Fact]
        public async Task PropCollectionDisplayname()
        {
            var db = MockDatabase();
            ...
            var propMethods = GetPropMethods(db);
            await propMethods.PropMethod(ColUrl, null, 0, props, response, await GetPrincipal(db));
            IXMLTreeStructure prop;
            response.GetChildAtAnyLevel("prop", out prop);
            Assert.NotNull(prop);
            Assert.True(prop.Children.Count == 1);
            Assert.Equal(prop.Children[0].Value as string, "C111");
        }

        [Fact]
        public async Task PropCollectionCalendarDescription()
        {
            var db = MockDatabase();
            ...
            Assert.NotNull(prop);
            Assert.True(prop.Children.Count == 1);
            Assert.Equal(prop.Children[0].Value as string, "No Description Available");
        }
...
//        [Fact]
//        public async Task PropCollectionGetcontenttype()
//        {
//            var db = MockDatabase();
//
...
//            var props = new List<KeyValuePair<string, string>>()
//            {
//                new KeyValuePair<string, string>("getcontenttype", "D")
//            };
//            await propMethods.PropMethod(ColUrl, null, 0, props, response, await GetPrincipal(db));
//            IXMLTreeStructure prop;
//            response.GetChildAtAnyLevel("prop", out prop);
//            Assert.NotNull(prop);
//            Assert.True(prop.Children.Count == 1);
//            Assert.Equal(prop.Children[0].Value as string, "text/calendar; component=vevent");
//        }

//         [Fact]
//         public async Task PropCollectionGetetag()
//         {
//             var db = MockDatabase();
//
//             var response = new XmlTreeStructure("multistatus", "DAV:");
...
//             Assert.NotNull(prop);
//             Assert.True(prop.Children.Count == 1);
//             Assert.Equal(prop.Children[0].Value as string, "0");
//         }

        [Fact]
        public async Task PropCollectionResourceType()
        {
            var db = MockDatabase();
            ...
            Assert.NotNull(prop);
            Assert.True(prop.Children.Count == 1);
            Assert.Equal(prop.Children[0].Children[0].NodeName as string, "collection");
            Assert.Equal(prop.Children[0].Children[1].NodeName as string, "calendar");
        }
        ...
        [Fact]
        public async Task ComparingCollectionFinalsXmlPropName()
        {
            var db = MockDatabase();
            var fs = new FileManagement();
            var calDav = GetCalDav(fs, db);
            ...
            requestHeaders.Headers.Add("Depth", "0");
            //await calDav.PropFind(prop, strBuilder.ToString(), response);
